...
class GeminiPlanillasService {
  ...
  async analyzeDocument(fileBuffer: ArrayBuffer, mimeType: string): Promise<any[]> {
    ...
    const prompt = `
      ...
      4. TEXTO FANTASMA O ERROR DE IMPRESIÓN: A veces la impresora deja números "fantasma" o repetidos pisando las líneas negras divisorias (por ejemplo, un PLU 3648773 borroso justo sobre una línea). ¡IGNORA POR COMPLETO cualquier texto que esté borroso, fantasma o montado sobre la línea negra! Solo extrae lo que esté claramente impreso en el centro del renglón de cada cliente.
      5. EXTRACCIÓN BASADA EN CUADRÍCULA (LÍNEAS FÍSICAS): Fíjate en las líneas horizontales que separan cada fila. Todo lo que esté en el mismo "renglón" físico pertenece al mismo registro. 
      6. HUECOS EN BLANCO: Si en el renglón físico de un cliente (ej. Maria Zaray o Isolina) la celda de PLU está vacía o solo tiene una mancha/texto fantasma sobre la línea, extrae OBLIGATORIAMENTE "N/A" para ese PLU. ¡NUNCA desplaces o "subas" un PLU del renglón de abajo para rellenar el hueco! 
      ...
      8. DESCRIPCIONES DE ARTÍCULOS: Copia el texto exacto. Si ves texto parcial o cortado, pon lo que alcances a leer. Solo pon "N/A" si la celda está completamente vacía.
      9. PROHIBIDO REPETIR VALORES CREADOS: Cada fila tiene su propio Pedido. Nunca inventes o dupliques el pedido de la fila de arriba a menos que sea visualmente idéntico.
