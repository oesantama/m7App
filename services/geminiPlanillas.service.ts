...
class GeminiPlanillasService {
  ...
  async analyzeDocument(fileBuffer: ArrayBuffer, mimeType: string): Promise<any[]> {
    ...
    const prompt = `
      ...
      4. EXTRACCIÓN BASADA EN CUADRÍCULA (LÍNEAS FÍSICAS): Fíjate en las líneas horizontales que separan cada fila. Todo lo que esté en el mismo "renglón" físico pertenece al mismo registro. 
      5. HUECOS EN BLANCO: Si en el renglón físico de un cliente la celda de PLU o Descripción está vacía, extrae OBLIGATORIAMENTE "N/A" para ese PLU. ¡NUNCA desplaces o "subas" un PLU del renglón de abajo para rellenar el hueco! Si el PDF tiene 13 pedidos y 2 clientes en el medio no tienen PLU impreso al lado, debes poner "N/A" a esos 2 clientes intermedios.
      6. SEPARACIÓN DE PEDIDO Y CÉDULA PEGADOS: Si ves un número extremadamente largo (ej. "163352206107970043502"), es porque el Pedido y la Cédula se imprimieron pegados. Sepáralos por lógica: los primeros ~13 dígitos son el Pedido ("1633522061079") y los últimos dígitos la Cédula ("70043502").
      7. DESCRIPCIONES DE ARTÍCULOS: Copia el texto exacto. Si ves texto parcial o cortado, pon lo que alcances a leer. Solo pon "N/A" si la celda está completamente vacía.
      8. PROHIBIDO REPETIR VALORES CREADOS: Cada fila tiene su propio Pedido. Nunca inventes o dupliques el pedido de la fila de arriba a menos que sea visualmente idéntico.
