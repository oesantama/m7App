    ...
    if (activeInvoice) {
        return (
            <div className="flex flex-col h-full bg-slate-50/50 rounded-[3rem] overflow-hidden border border-slate-200 shadow-2xl animate-in slide-in-from-right duration-500">
                {/* Header Detalle - COMPACTO */}
                <div className="px-5 py-3 bg-white border-b border-slate-100 flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-3">
                        <button onClick={() => { setActiveInvoice(null); setItemSearch(''); }} className="p-2 bg-slate-50 rounded-lg hover:bg-slate-100 transition-all border border-slate-200">
                            <Icons.ChevronRight className="w-3.5 h-3.5 rotate-180" />
                        </button>
                        <div>
                            <div className="flex items-center gap-2">
                                <p className="text-[7px] font-black text-slate-400 uppercase tracking-widest">Alistando Factura</p>
                                <span className="text-[6.5px] bg-emerald-50 text-emerald-600 px-1.5 rounded font-black border border-emerald-100 uppercase tracking-tighter">v2.5 FINAL</span>
                            </div>
                            <h3 className="text-sm font-black text-slate-900 uppercase tracking-tighter leading-none mt-0.5">
                ...
                {/* Buscador Interno - COMPACTO */}
                <div className="px-5 py-2.5 bg-slate-50/50 border-b border-slate-100 shrink-0">
                    <div className="relative group">
                        <Icons.Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 group-focus-within:text-emerald-500 transition-colors" />
                        <input 
                            type="text" 
                            placeholder="FILTRAR ARTÍCULOS (SKU O NOMBRE)..." 
                            value={itemSearch}
                            onChange={(e) => setItemSearch(e.target.value.toUpperCase())}
                            className="w-full pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-[9px] font-black text-slate-900 uppercase outline-none focus:border-emerald-500 shadow-sm transition-all"
                        />
                    </div>
                </div>
                ...
                <div className="px-8 py-4 bg-white border-t border-slate-100 shrink-0 flex justify-between items-center bg-gradient-to-r from-white to-slate-50">
                    ...
                    <button 
                        ...
                        className={`px-8 py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all shadow-xl ${isAllConfirmed ? 'bg-slate-950 text-white shadow-slate-900/20 active:scale-95 hover:bg-emerald-600' : 'bg-slate-100 text-slate-300 cursor-not-allowed'}`}
                    >
                        Guardar Alistado
                    </button>
                </div>
