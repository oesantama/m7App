...
const MOVEMENT_TYPE_LABELS: Record<string, { label: string; color: string }> = {
    ...
    ENTREGA_PARCIAL:   { label: 'Entrega Parcial',   color: 'bg-amber-100 text-amber-700' },
    DEVOLUCION_BODEGA: { label: 'Devolución',        color: 'bg-rose-100 text-rose-700' },
    SALIDA_PROVEEDOR:  { label: 'Salida Proveedor',  color: 'bg-slate-100 text-slate-600' },
    REPIQUE:           { label: 'Repique',            color: 'bg-cyan-100 text-cyan-700' },
...
const ConsultaInventario: React.FC = () => {
    ...
    const loadStock = async () => {
        setLoading(true);
        try {
            const res = await api.getInventoryStock({ location: 'all' });
    ...
    const loadMovements = async (p = 1) => {
        ...
        try {
            ...
            if (p === 1) setMovements(rows);
            else setMovements(prev => [...prev, ...rows]);
    ...
    const handleFilterMovements = () => loadMovements(1);
    ...
    return (
        <div className="min-h-screen bg-slate-50/50 p-4 md:p-6">
            ...
            <div className="mb-5 flex items-center justify-between">
                ...
                <button onClick={() => { loadStock(); loadMovements(1); }}
                    className="px-3 py-2 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-all">
            ...
            {loading && movements.length === 0 && bodegaRows.length === 0 ? (
                ...
                <>
                    {/* Filters */}
                    <div className="bg-white border border-slate-200 rounded-2xl p-4 mb-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
                        ...
                        <button onClick={handleFilterMovements}
                            className="col-span-2 md:col-span-1 px-3 py-1.5 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center justify-center gap-1.5">
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-[10px]">
                                        ...
                                        <tbody className="divide-y divide-slate-50">
                                            {movements.map(m => {
                                                const typeInfo = MOVEMENT_TYPE_LABELS[m.movement_type] ?? { label: m.movement_type, color: 'bg-slate-100 text-slate-500' };
                                                return (
                                                    <tr key={m.id} className="hover:bg-slate-50/50">
                                                        <td className="px-3 py-2 text-slate-500 whitespace-nowrap">{fmtDate(m.created_at)}</td>
                                                        <td className="px-3 py-2">
                                                            <span className={`px-1.5 py-0.5 rounded-full text-[8px] font-black ${typeInfo.color}`}>{typeInfo.label}</span>
                                                        </td>
...
const StockTable: React.FC<{ rows: StockRow[]; mode: 'bodega' | 'vehiculos' }> = ({ rows, mode }) => {
    ...
    const filtered = rows.filter(r => {
        ...
        return !q || (r.article_id ?? r.sku ?? '').toLowerCase().includes(q)
