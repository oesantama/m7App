...
Conteo ciego de inventario para validación sin sesgo de stock en bodega.

### Características principales

- Visualización de datos en tabla con paginación
- Formularios de captura y edición de información
- Búsqueda y filtros avanzados
- Exportación de datos (Excel / PDF)
- Diálogos y modales para acciones rápidas
- Navegación paginada de registros
...
2. En el menú lateral izquierdo, localice y haga clic en **Blind Count**.
...
### 4. Creación y edición de registros

1. Haga clic en el botón **Nuevo** o **Agregar** (ícono +) para crear un registro.
2. Complete todos los campos obligatorios marcados con `*`.
3. Revise la información ingresada antes de guardar.
4. Haga clic en **Guardar** o **Confirmar** para aplicar los cambios.
5. Para editar un registro existente, haga clic en el ícono de **Editar** (lápiz).
6. Para eliminar, confirme la acción en el diálogo de advertencia.
...
|-----------|------|-------------|
| `document` | `DocumentL` | Propiedad `document` |
| `user` | `User` | Propiedad `user` |
| `masterNotificaciones` | `MasterRecord[]` | Propiedad `masterNotificaciones` |
| `masterTipoNotificacion` | `MasterRecord[]` | Propiedad `masterTipoNotificacion` |
| `masterArticulo` | `MasterRecord[]` | Propiedad `masterArticulo` |
| `onConfirm` | `(finalItems` | Propiedad `onConfirm` |
| `onPartialSave` | `(currentItems` | Propiedad `onPartialSave` |
| `onCancel` | `() => void` | Propiedad `onCancel` |
| `onAddArticleToMaster` | `(article` | Propiedad `onAddArticleToMaster` |
| `onAddNotificationToMaster` | `(notif` | Propiedad `onAddNotificationToMaster` |
| `allowExtraItems?` | `boolean` | Propiedad `allowExtraItems?` |
