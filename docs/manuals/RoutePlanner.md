# Manual de Usuario — RoutePlanner
...
Planificación y optimización automática de rutas de entrega usando algoritmos ILS + Or-opt con integración OSRM.
...
2. En el menú lateral izquierdo, localice y haga clic en **Route Planner**.
...
|-----------|------|-------------|
| `invoices` | `Invoice[]` | Propiedad `invoices` |
| `vehicles` | `Vehicle[]` | Propiedad `vehicles` |
| `drivers` | `Driver[]` | Propiedad `drivers` |
| `assignments` | `VehicleAssignment[]` | Propiedad `assignments` |
| `documents` | `DocumentL[]` | Propiedad `documents` |
| `activeRoutes` | `Route[]; // Nueva prop` | Propiedad `activeRoutes` |
| `user` | `User` | Propiedad `user` |
| `clients` | `MasterRecord[]; // Nueva prop` | Propiedad `clients` |
| `onAssign` | `(vId` | Propiedad `onAssign` |
| `onSaveRoute` | `(route` | Propiedad `onSaveRoute` |
| `onRefresh?` | `() => void` | Propiedad `onRefresh?` |
